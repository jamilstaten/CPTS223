...
private:
	...
	void percolateDown(unsigned int hole)
	{
		...
		for (; (hole * 2) < size; hole = child) {
			child = hole * 2;
			if (child != size - 1 && _items[child + 1] < _items[child])
				child++;
	...
	{
		unsigned int hole = _items.size();
		_items.push_back(std::move(item));

		T toInsert = std::move(_items[hole]);

		for (; hole > 1 && toInsert < _items[hole / 2]; hole /= 2)
			_items[hole] = std::move(_items[hole / 2]);

		_items[hole] = std::move(toInsert);
	}
